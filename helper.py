...
def cconv(a, b):
    ...
    return torch.fft.irfft(com_mult(torch.fft.rfft(a, dim=1), torch.fft.rfft(b, dim=1)),n=a.shape[1], dim=1)

def ccorr(a, b):
	# return torch.fft.irfft(com_mult(conj(torch.fft.rfft(a, 1)), torch.fft.rfft(b, 1)), 1, signal_sizes=(a.shape[-1],))
	return torch.fft.irfft(com_mult(torch.conj(torch.fft.rfft(a)), torch.fft.rfft(b)), n=a.shape[-1])
...
#复数卷积操作
def ccorr(a, b):
	return torch.irfft(com_mult(conj(torch.rfft(a, 1)), torch.rfft(b, 1)), 1, signal_sizes=(a.shape[-1],))#复相关操作
