from helper import *
from message_passing import MessagePassing

class CompGCNConv(MessagePassing):
	def __init__(self, in_channels, out_channels, num_rels, act=lambda x:x, params=None):
		super(self.__class__, self).__init__()

		self.p 			= params
		self.in_channels	= in_channels
		self.out_channels	= out_channels
		self.num_rels 		= num_rels
		self.act 		= act #激活函数
		self.device		= None

		self.w_loop		= get_param((in_channels, out_channels))#自环连接的权重
		self.w_in		= get_param((in_channels, out_channels))#入边连接的权重
		self.w_out		= get_param((in_channels, out_channels))#出边连接的权重
		self.w_rel 		= get_param((in_channels, out_channels))#关系变换的权重
		self.loop_rel 		= get_param((1, in_channels));#自环关系的嵌入

		self.drop		= torch.nn.Dropout(self.p.dropout)#初始化dropout层
		self.bn			= torch.nn.BatchNorm1d(out_channels)#批归一化层

		if self.p.bias: self.register_parameter('bias', Parameter(torch.zeros(out_channels)))

	def forward(self, x, edge_index, edge_type, rel_embed): 
		if self.device is None:
			self.device = edge_index.device

		rel_embed = torch.cat([rel_embed, self.loop_rel], dim=0)#将自环关系添加到关系嵌入中
		num_edges = edge_index.size(1) // 2 #计算边的数量，假设每条边有两个方向（入边和出边）
		num_ent   = x.size(0)

		self.in_index, self.out_index = edge_index[:, :num_edges], edge_index[:, num_edges:]#入边和出边的索引
		self.in_type,  self.out_type  = edge_type[:num_edges], 	 edge_type [num_edges:]#入边和出边的类型
 		#自环连接的边索引和边类型
		self.loop_index  = torch.stack([torch.arange(num_ent), torch.arange(num_ent)]).to(self.device)
		self.loop_type   = torch.full((num_ent,), rel_embed.size(0)-1, dtype=torch.long).to(self.device)
       # #计算入边和出边的归一化系数
		self.in_norm     = self.compute_norm(self.in_index,  num_ent)
		self.out_norm    = self.compute_norm(self.out_index, num_ent)
		#分别计算入边、自环和出边的传播结果
		in_res		= self.propagate('add', self.in_index,   x=x, edge_type=self.in_type,   rel_embed=rel_embed, edge_norm=self.in_norm, 	mode='in')
		loop_res	= self.propagate('add', self.loop_index, x=x, edge_type=self.loop_type, rel_embed=rel_embed, edge_norm=None, 		mode='loop')
		out_res		= self.propagate('add', self.out_index,  x=x, edge_type=self.out_type,  rel_embed=rel_embed, edge_norm=self.out_norm,	mode='out')
		#将入边、自环和出边的结果进行加权平均
		out		= self.drop(in_res)*(1/3) + self.drop(out_res)*(1/3) + loop_res*(1/3)
        #添加偏置并应用批归一化
		if self.p.bias: out = out + self.bias
		out = self.bn(out)

		return self.act(out), torch.matmul(rel_embed, self.w_rel)[:-1]	#返回激活后的输出和变换后的关系嵌入(去掉自环部分)	# Ignoring the self loop inserted
	
    #关系变换方法，支持三种操作模式：相关、减法和乘法
	def rel_transform(self, ent_embed, rel_embed):
		if   self.p.opn == 'corr': 	
			trans_embed  = ccorr(ent_embed, rel_embed)
		elif self.p.opn == 'sub': 	
			trans_embed  = ent_embed - rel_embed
		elif self.p.opn == 'mult': 	
			trans_embed  = ent_embed * rel_embed
		elif self.p.opn == "corr_new":
			trans_embed = ccorr_new(ent_embed, rel_embed)
		elif self.p.opn == "conv":
			trans_embed = cconv(ent_embed, rel_embed)
		elif self.p.opn == "conv_new":
			trans_embed = cconv_new(ent_embed, rel_embed)
		elif self.p.opn == 'cross':
			trans_embed = ent_embed * rel_embed + ent_embed
		elif self.p.opn == "corr_plus":
			trans_embed = ccorr_new(ent_embed, rel_embed) + ent_embed
		elif self.p.opn == "rotate":
			trans_embed = rotate(ent_embed, rel_embed)
		else: raise NotImplementedError

		return trans_embed
    #消息传递函数，根据模式选择权重，应用关系变换和归一化
	def message(self, x_j, edge_type, rel_embed, edge_norm, mode):
		weight 	= getattr(self, 'w_{}'.format(mode))
		rel_emb = torch.index_select(rel_embed, 0, edge_type)
		xj_rel  = self.rel_transform(x_j, rel_emb)
		out	= torch.mm(xj_rel, weight)

		return out if edge_norm is None else out * edge_norm.view(-1, 1)

	def update(self, aggr_out):
		return aggr_out
    #计算归一化系数，使用度矩阵的-0.5次方
	def compute_norm(self, edge_index, num_ent):
		row, col	= edge_index
		edge_weight 	= torch.ones_like(row).float()
		deg		= scatter_add( edge_weight, row, dim=0, dim_size=num_ent)	# Summing number of weights of the edges
		deg_inv		= deg.pow(-0.5)							# D^{-0.5}
		deg_inv[deg_inv	== float('inf')] = 0
		norm		= deg_inv[row] * edge_weight * deg_inv[col]			# D^{-0.5}

		return norm

	def __repr__(self):
		return '{}({}, {}, num_rels={})'.format(
			self.__class__.__name__, self.in_channels, self.out_channels, self.num_rels)
