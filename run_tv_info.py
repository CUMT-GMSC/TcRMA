...
class Runner(object):
	...
	def load_data(self):
		...
		ent_set, rel_set = OrderedSet(), OrderedSet() #使用 OrderedSet 来存储实体和关系
		for split in ['train', 'test', 'valid']:
			for line in open('./data/{}/{}.txt'.format(self.p.dataset, split)):
				sub, rel, obj = map(str.lower, line.strip().split('\t')) #头实体 关系 尾实体
				ent_set.add(sub)
				rel_set.add(rel)
				ent_set.add(obj)
		 #创建映射 实体2id 关系2id
		...
		self.p.num_rel		= len(self.rel2id) // 2 # 每个关系都有一个反向关系，所以除以2
		# self.p.num_rel		= len(self.rel2id)
		self.p.embed_dim	= self.p.k_w * self.p.k_h if self.p.embed_dim is None else self.p.embed_dim #嵌入维度
		
		# 确保维度一致性：统一使用embed_dim
		...
		sr2o = ddict(set)#训练集中的 (sub, rel) -> obj 映射
	...
	def __init__(self, params):
		...
		# self.scheduler    = self.add_scheduler(self.optimizer)
	...
	# def add_scheduler(self, optimizer):
	# 	"""
	# 	Creates a learning rate scheduler for training
	...
	# 	"""
	# 	return torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=self.p.lr_decay)
	...
	# def calc_orth_loss(self):
	# 	"""
	# 	只计算 Memory Bank 的正交损失
	# 	目标：让不同的 Memory Slot 向量之间尽可能垂直（互不相关），
	# 	"""
	# 	loss_orth = torch.tensor(0.0, device=self.device)
	# 	found = False
		
	# 	# 自动遍历模型参数，找到 memory_bank
	# 	for name, param in self.model.named_parameters():
	# 		if 'memory_bank' in name:
	# 			mem = param
	# 			found = True
				
	# 			#  归一化 (Normalize): 消除向量模长的影响，只看方向
	# 			mem_norm = torch.norm(mem, p=2, dim=1, keepdim=True)
	# 			mem_normalized = mem / (mem_norm + 1e-9)
				
	# 			# 计算 Gram 矩阵 (M * M^T)
	# 			# 如果完全正交，结果应该是单位矩阵 Identity
	# 			gram_matrix = torch.mm(mem_normalized, mem_normalized.t())
				
	# 			# 创建目标单位矩阵
	# 			identity = torch.eye(mem.size(0)).to(self.device)
				
	# 			#  计算差异 (Frobenius Norm)
	# 			loss_orth += torch.norm(gram_matrix - identity, p='fro')
		
	# 	if not found:
	# 		print("Warning: 'memory_bank' parameter not found in model!")
	...
	def run_epoch(self, epoch, val_mrr = 0):
		...
		for step, batch in enumerate(train_iter):
			...
			# orth_loss = self.calc_orth_loss()
			# #平滑退火策略
			# if epoch < T_start:
			# 	lambda_orth = 0.0
			# elif epoch > T_end:
			# 	lambda_orth = max_lambda
			# else:
			#  # 线性增长
			# 	lambda_orth = max_lambda * ((epoch - T_start) / (T_end - T_start))

			# loss = main_loss + lambda_orth * orth_loss
			loss = main_loss 
			loss.backward()
			self.optimizer.step()
...
#每个 epoch 后进行一次训练和验证，若验证集 MRR提高，保存当前模型； 如果连续 25 次验证集 MRR 未提升，则停止训练
	def fit(self):
		"""
...
if __name__ == '__main__':
	...
	parser.add_argument('-name',		default='tv_gat_info_memory_fb',					help='Set run name for saving/restoring models')
	parser.add_argument('-data',		dest='dataset',         default='FB15k-237',            help='Dataset to use, default: FB15k-237  WN18RR')
	parser.add_argument('-model',		dest='model',		default='tv_gat_info_memory',		help='Model Name')
	parser.add_argument('-score_func',	dest='score_func',	default='conve',		help='Score Function for Link prediction')
	parser.add_argument('-opn',             dest='opn',             default='corr_new',                 help='Composition Operation to be used in CompGCN')
	#combine_type corr_new
	...
	parser.add_argument('-num_memories', dest='num_memories', default=203, type=int, help='Number of global memory prototypes')#关系总数的1/10--1/5
	#umls 46关系
	...
	parser.add_argument('-lr',		type=float,             default=0.0015,			help='Starting Learning Rate')
	...
	parser.add_argument('-gnn_input_dropout',	dest='gnn_input_dropout', 	default=0.3,  	type=float,	help='')
	...
	parser.add_argument('-ConvE_hid_drop',  	dest='ConvE_hid_drop', 	default=0.3,  	type=float,	help='ConvE: Hidden dropout')
	parser.add_argument('-ConvE_feat_drop', 	dest='ConvE_feat_drop', 	default=0.4,  	type=float,	help='ConvE: Feature Dropout')
